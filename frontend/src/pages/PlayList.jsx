...
const PlayList = ({ user }) => {
  const { songs, setSelectedSong, setIsPlaying, fetchAlbums, fetchSongs } = SongData();
  ...
  return (
    <Layout>
      ...
      {myPlaylist &&
        myPlaylist.map((e, i) => (
          <div
            className="grid grid-cols-3 sm:grid-cols-4 mt-10 mb-4 pl-2 text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer  items-center"
            key={i}
          >
            <p className="text-white">
              <span className="mr-4 text-[#a7a7a7]">{i + 1}</span>
